...
componentDidMount(){
    ...
    this.props.getProposalOrderDetail_(this.props.navigation.getParam('prop_id'),'','');
}
payBtnHandle(){
    var data = {
        id:this.state.id,
...
applyBtnHandle = ()=>{
    ...
    this.props.getProposalOrderDetail_(this.state.prop_id,this.state.odr_id,this.state.coupon);
}
componentWillReceiveProps(nextProps){
    if(nextProps.state.proposalDetails.proposalDetails!=null && nextProps.state.proposalDetails.error==null){
        ...
        var GridViewItems = [
            ...
            {key: '$' + this.convertTotal(nextProps.state.proposalDetails.proposalDetails.order_details.odr_pkg_price),id : 2},
            {key: 'Coupon Discount',id:1},
            {key: '$' + this.convertTotal(coupon_discount),id:4},
            {key: 'Tax',id:1},
            {key: 'No taxable items found in your order',id:2},
            {key: 'Subtotal',id:1},
            {key: '$' + this.convertTotal(nextProps.state.proposalDetails.proposalDetails.order_details.odr_total_value),id:2},
            {key: 'Shipping Charges',id:1},
            {key: '$' + this.convertTotal(nextProps.state.proposalDetails.proposalDetails.order_details.odr_pkg_shipping),id:2},
            {key: 'Total',id:1},
            {key: '$' + this.convertTotal(nextProps.state.proposalDetails.proposalDetails.order_details.odr_total_outstanding),id:2}
        ];
    ...
    }else if(nextProps.state.proposalDetails.proposalDetails==null && nextProps.state.proposalDetails.error!=null){
                    ...
                    <View style={{flex:4,alignItems:'center'}}>
                        <TouchableOpacity 
                            ...
                            <Text style = {{fontSize:10, color:'white'}}>
                                PAY NOW ${this.convertTotal(this.state.amount)}
                            </Text>
                        </TouchableOpacity>
                    </View>
                </View>        
...
function mapDispatchToProps(dispatch){
	return{
		getProposalOrderDetail_ : (prop_id,odr_id,coupon)=>{
			dispatch(getProposalOrderDetail(prop_id,odr_id,coupon));
    }
  }	
}
